...
class ForEachListTests {
    ...
    private val initialParentState = ParentListState(
        parentText = "parent",
    ...
    private val parentReducer = Reducer<ParentListState, ParentAction> { state, action ->
            when (action) {
    ...
    private val mergedReducer: Reducer<ParentListState, ParentAction> = parentReducer.forEachList(
    ...
    fun `parent reducer should reduce on top of already reduced state by element reducer`() = runTest {
        mergedReducer.testReduceState(
            ...
            state shouldBe initialParentState.copy(
                ...
                elements = initialParentState.elements.toMutableList().apply {
